...
from clustering import get_representative_examples, cluster_with_kmeans
...
def rename_cluster_state(
    ...
    examples = get_representative_examples(data=data, cluster_id=cluster_id, samples_per_tercile=2)
...
def binary_merge_cluster_state(
    ...
    examples = get_representative_examples(data=data, cluster_id=new_cluster_id, samples_per_tercile=2)
    result = name_cluster(llm_service=llm_service, examples=examples, reason=reason)
...
def split_cluster_state(
    ...
    data.loc[cluster_indices[new_labels == 0], "cluster_id"] = new_cluster_id_1
    data.loc[cluster_indices[new_labels == 1], "cluster_id"] = new_cluster_id_2

    del cluster_metadata[cluster_id]

    examples_1 = get_representative_examples(data=data, cluster_id=new_cluster_id_1, samples_per_tercile=2)
    examples_2 = get_representative_examples(data=data, cluster_id=new_cluster_id_2, samples_per_tercile=2)

    result_1 = name_cluster(llm_service=llm_service, examples=examples_1, reason=reason)
    result_2 = name_cluster(llm_service=llm_service, examples=examples_2, reason=reason)
